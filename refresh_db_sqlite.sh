...
# mysql:
time for i in `echo "SHOW TABLES;" | ./manage.py dbshell`; do echo "DROP TABLE IF EXISTS $i;" | ./manage.py dbshell; done
echo "3. recreate db"
...
# MariaDB
time for i in `echo "SHOW TABLES;" | ./manage.py dbshell`; do echo "DELETE FROM $i;" | ./manage.py dbshell; done
echo "5. load data"
